...
#include <ControlInterface.h>  // Definition of thrift interface
...
int main(int argc, char *argv[])
{
	...
	yarp::os::Property parameter;
	parameter.fromConfigFile(argv[2]); // Load the properties from the config file
	...
	for (int i = 0; i < names.size(); i++)
		commandList.emplace(names[i], 0); // Put them in to the map

	// Load the list of Cartesian actions
	bottle->clear();
	bottle = parameter.findGroup("CARTESIAN_ACTIONS").find("names").asList();
	names = string_from_bottle(bottle);
	for (int i = 0; i < names.size(); i++)
		commandList.emplace(names[i], 1);

	// Load the list of grasp actions
	bottle->clear();
	bottle = parameter.findGroup("GRASP_ACTIONS").find("names").asList();
	names = string_from_bottle(bottle);
	for (int i = 0; i < names.size(); i++)
		commandList.emplace(names[i], 2);
	...
	{
		std::cerr << errorMessage << "Could not connect '/commandClient' with '" << serverPortName << "'.\n";
		return 1;
	}

	ControlInterface client;
	...
	while (active)
	{
		...
		else
		{
			...
			if (blah == commandList.end())
			{
				...
				{
					if (client.perform_grasp_action(command))
			...
			else
			{
				...
				{
					if (client.perform_joint_space_action(command))
						output.addString("Capito");
					else
						output.addString("Problema");
				}
				...
				{
					if (client.perform_cartesian_action(command))
						output.addString("Capito");
					else
						output.addString("Problema");
				}
